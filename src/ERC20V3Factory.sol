...
interface IStdERC20 {
    function init(address _creator, uint256 _totalSupply, string memory _name, string memory _symbol, uint8 _decimals)
        external;
}
...
contract ERC20V3Factory is InitializableOwnable {
    ...
    function createStdERC20(uint256 totalSupply, string memory name, string memory symbol, uint8 decimals)
        external
        payable
        returns (address newERC20)
    {
    ...
    function createCustomERC20(
        ...
        ICustomERC20(newCustomERC20).init(
            msg.sender, totalSupply, name, symbol, decimals, tradeBurnRatio, tradeFeeRatio, teamAccount
        );
    ...
    function createCustomMintableERC20(
        ...
        ICustomERC20(newCustomMintableERC20).init(
            msg.sender, initSupply, name, symbol, decimals, tradeBurnRatio, tradeFeeRatio, teamAccount
        );
